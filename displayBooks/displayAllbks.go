...
//DisplayListOfbook **
func DisplayListOfbook(w http.ResponseWriter, r *http.Request) {
	if	validated := s.GetSession(r);!validated{
		...
		qr, err := db.Query(`select * from book_instances;`)
		if err != nil {
			panic(err)
		}
		...
		if vars.Email == lbemail {
			vars.Tpl.ExecuteTemplate(w, "ListOfBooks.html", books)
			return
		}
		vars.Tpl.ExecuteTemplate(w, "m_listOfBooks.html", books)
		return
}