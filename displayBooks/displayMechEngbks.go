...
//DisplayMechEngBooks **
func DisplayMechEngBooks(w http.ResponseWriter, r *http.Request) {

	if	validated := s.GetSession(r);!validated{
		http.Redirect(w, r, "/home", http.StatusSeeOther)
		return
	}
		vars.Subject = "Mechanical Engineering"
		...
		qr, err := db.Query(`select * from book_instances where subject_area = 'Mechanical Engineering';`)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
		}
		...
		if os.Getenv("EMAIL") == lbemail {
			vars.Tpl.ExecuteTemplate(w, "booksMechEng.html", books)
			return
		}
		vars.Tpl.ExecuteTemplate(w, "m_booksMechEng.html", books)
		return
}