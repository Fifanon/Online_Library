...
//DisplayElecBooks **
func DisplayElecBooks(w http.ResponseWriter, r *http.Request) {

	if	validated := s.GetSession(r);!validated{
		http.Redirect(w, r, "/home", http.StatusSeeOther)
		return
	}
		vars.Subject = "Electronic Engineering"
		...
		qr, err := db.Query(`select * from book_instances where subject_area = 'Electronic Engineering';`)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
		}
		...
		if os.Getenv("EMAIL") == lbemail {
			vars.Tpl.ExecuteTemplate(w, "booksElecEng.html", books)
			return
		}
		vars.Tpl.ExecuteTemplate(w, "m_booksElecEng.html", books)
		return
}